...
def pos2catid(ra, dec):
    ...
    mask = (ra>=rmin) & (ra<rmax) & (dec>=dmin) & (dec<dmax) & (depth=='\x04')
...
def search_radius_objid( ra, dec, radius=5. ):
    ...
    dist = _greatcirdist(np.float64(ra), np.float64(dec),
        cpt['RA'].astype('float64'), cpt['DEC'].astype('float64'))
    mask = (dist < radius)
...
def id2epoch_data(catid, objid, system='psf', tformat='unix', grouped=False):
    ...
    phot = phot - 25.0 + clam_data*0.001 + Kcorr_data*(airmass-1.0) - mcal
    ...
    if grouped == True:
        mask = [(photcode>=10000+i*100)&(photcode<10100+i*100) for i in range(5)]
        return [(ra[i], dec[i], phot[i], photerr[i], photcode[i], epoch[i]) for i in mask]
    else:
